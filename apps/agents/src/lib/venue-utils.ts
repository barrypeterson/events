...
/**
 * Find existing venues that match the given venue data
 * Uses multiple strategies: exact name, address, coordinates, fuzzy name
 */
export async function findMatchingVenues(venueData: {
  ...
  // Strategy 3: Same coordinates (within 50 meters)
  ...
  // Strategy 4: Fuzzy name match (similarity > 0.85)
...
/**
 * Check if a venue has been merged into another
 */
export async function getCanonicalVenue(venueId: string): Promise<string> {
  ...
  if (duplicate) {
    // Recursively find the canonical venue (in case of chains)
    return getCanonicalVenue(duplicate.canonicalVenueId);
  }
