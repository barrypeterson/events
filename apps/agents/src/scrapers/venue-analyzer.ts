...

{
  ...
  "extractionPrompt": "A focused system prompt for gpt-4o-mini that tells it exactly how to extract events from this page's text. CRITICAL: The prompt MUST instruct the model to return a JSON array where each event has these fields: title, rawDate, rawTime, rawDescription, rawPrice, imageUrl (from [IMAGE: url] markers, null if none), url (ticket purchase link — often off-site like ticketmaster.com, axs.com, eventbrite.com, prekindle.com — or null if not present), detailUrl (the VENUE'S OWN detail page for this event on ${sourceUrl.match(/https?:\/\/[^/]+/)?.[0] ?? 'the venue domain'} — typically a [LINK: ...] to a page under the same hostname as the source. null if the only link points off-site). The prompt MUST make clear that detailUrl and url are SEPARATE: detailUrl is same-origin (the venue's page about the event), url is the ticket/purchase link (often third-party). The prompt MUST say 'Return ONLY a JSON array, no markdown, no explanations.' Include 1-2 concrete example snippets copied verbatim from the page text above so the model knows what a single event block looks like, and at least one example showing a detailUrl value. 250-600 words max.",
