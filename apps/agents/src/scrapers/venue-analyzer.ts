...

{
  ...
  "extractionPrompt": "A focused system prompt for gpt-4o-mini that tells it exactly how to extract events from this page's text. CRITICAL: The prompt MUST instruct the model to return a JSON array where each event has these fields: title, rawDate, rawTime, rawDescription, rawPrice, imageUrl (extract the URL VALUE from [IMAGE: url] markers — output only the bare https:// URL, NEVER include the literal '[IMAGE: ' prefix or the trailing ']'; null if no image), url (extract the URL VALUE from a [LINK: url] marker for ticket/purchase — often off-site like ticketmaster.com, axs.com, eventbrite.com, prekindle.com — output only the bare https:// URL, NEVER include the marker brackets; null if not present), detailUrl (extract the URL VALUE from the [LINK: url] marker pointing to the VENUE'S OWN detail page for this event on ${sourceUrl.match(/https?:\/\/[^/]+/)?.[0] ?? 'the venue domain'} — output only the bare https:// URL, NEVER include the marker brackets. null if the only link points off-site). The prompt MUST make clear that detailUrl and url are SEPARATE: detailUrl is same-origin (the venue's page about the event), url is the ticket/purchase link (often third-party). The prompt MUST say 'Return ONLY a JSON array, no markdown, no explanations.' THE PROMPT MUST WARN: 'For imageUrl, url, and detailUrl: emit only the bare URL string (e.g. \"https://example.com/img.jpg\"). DO NOT include the [IMAGE: or [LINK: prefix or the closing ] — those are markers in the input, not part of the URL.' Include 1-2 concrete example snippets copied verbatim from the page text above so the model knows what a single event block looks like, with a worked example showing input '[IMAGE: https://x.com/y.jpg]' → output 'https://x.com/y.jpg' (no brackets). 300-700 words max.",
