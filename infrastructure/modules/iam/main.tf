# IAM Module - Roles and policies for ECS, Lambda, and other services
    ...
    Statement = [
      {
        ...
        Resource = "arn:aws:secretsmanager:${var.region}:*:secret:${local.name_prefix}*"
      },
      {
        Effect = "Allow"
        Action = [
  ...
  policy = jsonencode({
    ...
    Statement = [
      ...
      },
      {
        ...
        Resource = "arn:aws:logs:${var.region}:*:log-group:/ecs/${local.name_prefix}*"
      },
      {
        Effect = "Allow"
        Action = [
...
resource "aws_cloudwatch_log_resource_policy" "vpc_flow_logs" {
  ...
  policy_name = "${local.name_prefix}-vpc-flow-logs-policy"
